...
function renderUsuariosRoles(data) {
    ...
    $j.each(data, function (i, usuarioRole) {
        ...
        tbody.append('<tr id="' + sanitizedId + '">' +
            ...
            '<button class="btn btn-warning" onclick="desactivarUsuarioRole(\'' + usuarioRole.correoElectronico + '\')"><i class="fas fa-user-slash"></i></button>' +
            '</td>' +
            '</tr>');
    });
...
function desactivarUsuarioRole(correoElectronico) {
    console.log('URL:', 'https://localhost:7280/api/UsuarioRole/DesactivarUsuario/' + encodeURIComponent(correoElectronico));

    Swal.fire({
        title: "¿Estás seguro?",
        text: "Una vez desactivado, el usuario no podrá acceder al sistema.",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            $j.ajax({
                url: 'https://localhost:7280/api/UsuarioRole/DesactivarUsuario/' + encodeURIComponent(correoElectronico),
                method: 'PUT',
                success: function () {
                ...
                error: function (xhr, status, error) {
                    console.error('Error desactivando UsuarioRole:', error);
